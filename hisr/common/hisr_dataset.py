...
class HISRSession():
    ...
    def get_dataloader(self, dataset_name, distributed, state_dataloader):
        ...
        if dataset_type == "Hdf5":
            if dataset_name in ["cave_x4", "harvard_x4", "cave_x8", "harvard_x8", "GF5_GF1"]:
        ...
        dataloader = \
            DataLoader(dataset, batch_size=self.samples_per_gpu,
                       ...
                       shuffle=(sampler is None), num_workers=self.workers_per_gpu, drop_last=False,
                       sampler=sampler)
    ...
    def get_eval_dataloader(self, dataset_name, distributed):
        ...
        #     else:
        #         raise NotImplementedError(f"{dataset_name} is not supported")
        if dataset_name in ["cave_x4", "harvard_x4", "cave_x8", "harvard_x8", "GF5-GF1"]:
            dataset = DatasetFromHdf5(getattr(self.args.dataset, f"{dataset_name}_test_path"))
        else:
            print(f"{dataset_name} is not supported.")
            raise NotImplementedError
