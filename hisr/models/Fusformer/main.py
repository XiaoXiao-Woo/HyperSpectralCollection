...
model_folder = "Trained_model/cave_x4/"
...
def train(training_data_loader, validate_data_loader, start_epoch=0, RESUME=False):
    ...
    if RESUME:
        path_checkpoint = model_folder+"{}.pth".format(450)
...
def test():
    test_set = DatasetFromHdf5("F:/Data/HSI/cave_x4/test_cavepatches128-2.h5")
    ...
    for iteration, batch in enumerate(testing_data_loader, 1):
        ref,  LRHSI, HRMSI = Variable(batch[0]), Variable(batch[1]), Variable(batch[2])
        LRHSI = LRHSI.cuda()
        HRMSI = HRMSI.cuda()
        with torch.no_grad():
    ...
    sio.savemat('harvard10-fusformer.mat',{'output': output_HRHSI})
...
if __name__ == "__main__":
    ...
    if train_or_not:
        ...
        train_set = DatasetFromHdf5('F:/Data/HSI/cave_x4/train_cave(with_up)x4.h5')  # creat data for training
        training_data_loader = DataLoader(dataset=train_set, num_workers=0, batch_size=batch_size, shuffle=True,
                                          pin_memory=True, drop_last=True)  # put training data to DataLoader for batches
        validate_set = DatasetFromHdf5('F:/Data/HSI/cave_x4/validation_cave(with_up)x4.h5')  # creat data for validation
