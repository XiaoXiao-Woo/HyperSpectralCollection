...
def zero_pad(image, shape, position='corner'):
    ...
    # imshape = torch.tensor(image.shape)
    ...
    # pad_img = torch.zeros(list(shape), dtype=image.dtype)

    idx, idy = np.indices(imshape)
    # a = torch.linspace(0,imshape[1]-1,imshape[1])

    # idy = a.repeat((8,1)).int()
    # idx = idy.t()
    ...
    # pad_img[offx:imshape[0] + offx, offy:imshape[1] + offy] = image
    return pad_img
