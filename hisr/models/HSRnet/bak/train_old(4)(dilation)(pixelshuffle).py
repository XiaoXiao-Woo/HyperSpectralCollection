...
def rgbNet(ms, RGB, num_spectral=31, num_res=6, num_fm=64, reuse=False):
    ...
    with tf.variable_scope('net'):
        ...
        rsp1, rsp2 = tf.split(rs, [15, 16], axis=3)
        Rslice, Gslice, Bslice = tf.split(RGB, 3, axis=3)
        ...
        for i in range(num_res):
            rs1 = ly.conv2d(rs, num_outputs=num_fm, kernel_size=3, stride=1,rate=2,
                            weights_regularizer=ly.l2_regularizer(weight_decay),
                            weights_initializer=ly.variance_scaling_initializer(), activation_fn=tf.nn.leaky_relu)
            rs1 = ly.conv2d(rs1, num_outputs=num_fm, kernel_size=3, stride=1,rate=2,
                            weights_regularizer=ly.l2_regularizer(weight_decay),
...
def train():
    ...
    train_data_name = 'G:\JF_Hu/1-HSRnet\DATA/train4(20-11)(pRGB).mat'  # training data
    test_data_name = 'G:\JF_Hu/1-HSRnet\DATA/validation4(20-11)(pRGB).mat'  # validation data
    ...
    with tf.Session() as sess:
        ...
        rgb1 = train_data['rgb1'][...]  #### rgb image N*H*W
        ...
        rgb2 = test_data['rgb1'][...]  #### rgb image N*H*W
        ...
        file = open('train_mse_models(rgb+RGB-10res-64fm)(20-11).txt',
                    'w')  # write the training error into train_mse.txt
        ...
        file = open('train_time_models(rgb+RGB-10res-64fm)(20-11).txt',
                    'w')  # write the training error into train_mse.txt
...
def test():
    ...
    test_data = 'G:\JF_Hu/1-HSRnet\DATA/test_cave_11(pRGB).mat'
    ...
    # lms = lms[np.newaxis,:,:,:]
    rgb = data['rgb1'][...]  # rgb image: 64x64x3
    ...
    with tf.Session() as sess:
        ...
        time_e = time.time()
        print(time_e - time_s)
        sio.savemat('output-models(6res_slice_old)(pixelshuffle)(leaky_relu)scale4.mat',
                    {'output': final_output})
if __name__ =='__main__':
    global model_directory
    model_directory = './3_models/models(6res_slice_old)(pixelshuffle)(leaky_relu)scale4'
    ##声明全局变量
    # train()
    print('-------------testing--------------')
    test()
