...
class DCT(nn.Module):
    def __init__(self, hsi_channel, msi_channel, upscale_factor, n_feats=180):
        super(DCT, self).__init__()
        kernel_size = 3
        self.up_factor = upscale_factor
        n_colors = hsi_channel
        self.headX = nn.Conv2d(n_colors, n_feats, kernel_size, stride=1, padding=3 // 2)
        self.headY = nn.Sequential(
            nn.Conv2d(msi_channel, 64, kernel_size, stride=1, padding=3 // 2),
            nn.ReLU(),
    ...
    def forward(self, x, y):
        x = torch.nn.functional.interpolate(
            x, scale_factor=self.up_factor, mode="bicubic", align_corners=False
        )
        x = self.headX(x)
        res = x
        y = self.headY(y)
