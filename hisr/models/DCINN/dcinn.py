...
class SELayer(torch.nn.Module):
    def __init__(
        self, num_filter, kernel_size=3, stride=1, padding=1, bias=True, norm=None
    ):
...
class Res(nn.Module):
    def __init__(self, n_feat, groups=1):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv2d(
                n_feat, n_feat, kernel_size=3, stride=1, bias=True, groups=groups
            ),
...
class CNet(torch.nn.Module):
    def __init__(
        ...
        self.in_conv = nn.Conv2d(
            num_channel + num_spectral, base_filter, 3, 1, 0, bias=True
        )
        self.tailh = nn.Conv2d(base_filter, base_filter // 2, 3, 1, 0, bias=True)
        self.tail = nn.Conv2d(base_filter, base_filter // 2, 3, 1, 0, bias=True)
        self.up = nn.Upsample(scale_factor=scale_factor, mode="bilinear")
        self.down = nn.Upsample(scale_factor=1 / 2, mode="bilinear")
        body = [Res(base_filter) for _ in range(num_block)]
...
class MyBlock(nn.Module):
    def __init__(self, channel_in, channel_out, gc=48, mshape=32, bias=True):
        ...
        self.conv3 = nn.Sequential(
            nn.ReflectionPad2d(1), nn.Conv2d(gc, channel_out, 3, 1, 0, bias=bias)
        )
        self.ca = GN(gc)
...
class CInvBlock(nn.Module):
    ...
    def forward(self, x, m, rev=False):
        if not rev:
            ...
            self.s = self.clamp_s * torch.sigmoid(
                self.H(torch.cat((y1, m), dim=1) * 2 - 1)
            )
            ...
            self.s = self.clamp_s * torch.sigmoid(
                self.H(torch.cat((x1, m), dim=1) * 2 - 1)
            )
...
class DCINN(nn.Module):
    def __init__(self, channel_in=3, channel_out=3, block_num=8):
        ...
        self.inv_in = CInvBlock(31, 31 // 2, gc=32, mshape=32)
        for j in range(block_num):
            b = CInvBlock(channel_num * 4, channel_split_num * 4, gc=64, mshape=32)
            operations.append(b)
        self.operations = nn.ModuleList(operations)
        self.inv_out = CInvBlock(31, 31 // 2, gc=32, mshape=32)
        self.har_in = HaarDownsampling(31)
        self.har_out = HaarDownsampling(31)
        self.c_net = CNet()
        self.spe_mlp = nn.Conv2d(3, 31, 1, 1, 0, bias=False)
        self.R = read_r().cuda()
        # self.spe_mlp.weight.data = self.R[None].permute(3, 2, 1, 0)
        # self.spe_mlp.requires_grad_ = False

    def forward(self, x, y, z):
        """
        x: before (1, 3, 64, 64), now (1, 31, 16, 16)
        y: (1, 31, 16, 16)
        z: (1, 3, 64, 64)
        """
        m1, m = self.c_net(y, z)

        # x = F.interpolate(y, scale_factor=4, mode="bilinear")  # MS: 32x32x31
        x = self.spe_mlp(z) - y
    ...
    def train_step(self, data, **kwargs):
        sr = self(data["rgb"], data["up"], data["rgb"]) + data["up"]

        loss_dicts = self.criterion(sr, data["gt"])

        return loss_dicts

    def val_step(self, data, **kwargs):
        return self(data["rgb"], data["up"], data["rgb"]) + data["up"]


def read_r():
    r_path = "/home/dsq/NIPS/HyperSpectralCollection/hisr/models/DCINN/D700-pinv-r/DCINND700-pinv-r.mat"
    data = sio.loadmat(r_path)
    r = data["r"][...]
    r = np.array(r, dtype=np.float32)
    R = torch.from_numpy(r)
...
if __name__ == "__main__":
    ...
    f_model = DCINN(channel_in=31, channel_out=31, block_num=4).cuda()
    gt = torch.randn(1, 31, 64, 64).cuda()
    MS = torch.randn(1, 3, 64, 64).cuda()
    HS = torch.randn(1, 31, 16, 16).cuda()

    MS0 = MS
    HS0 = HS
    HS = torch.nn.functional.interpolate(
        HS, scale_factor=4, mode="bilinear"
    )  # MS: 32x32x31

    HS1 = torch.randn(1, 31, 64, 64).cuda()  # HS @ r 3->31

    # print(f_model(HS, HS1, MS0).shape)

    print(flop_count_table(FlopCountAnalysis(f_model, (HS1 - HS, HS1, MS0))))
