...
def window_partition_shuffle(x, window_size):
    """
    Args:
        x: (B, H, W, C)
        window_size (int): window size
    ...
    Returns:
        windows: (num_windows*B, window_size, window_size, C)
    """
    x = rearrange(x, 'b (ws1 hh) (ws2 ww) c -> (b hh ww) (ws1 ws2) c', ws1=window_size, ws2=window_size)
    return x

def window_reverse_shuffle(windows, window_size, H, W):
    """
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size
    ...
    """
    B = int(windows.shape[0] / (H * W / window_size / window_size))
    x = rearrange(windows, '(b hh ww) ws1 ws2 d -> b (ws1 hh) (ws2 ww) d',  b=B, hh=H // window_size, ws1=window_size, ws2=window_size)
    return x
...
class Window_Attention_shuffle(nn.Module):
    ...
    def __init__(self, dim=32, input_resolution=16, num_heads=8, window_size=4,
        ...
        self.input_resolution = to_2tuple(input_resolution)
        self.num_heads = num_heads
        self.window_size = input_resolution//window_size
        self.mlp_ratio = mlp_ratio
        self.win = win
    ...
    def forward(self,H, W, x):
        ...
        x = self.norm1(x)
        x = x.view(B, H, W, C)
        x_windows = window_partition_shuffle(x, self.window_size)  # nW*B, window_size*window_size, C
        ...
        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, C)
        x = window_reverse_shuffle(attn_windows, self.window_size, H, W)  # B H' W' C
        x = x.view(B, H * W, C)
        # FFN
        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class Stage(nn.Module):
    def __init__(self, dim=32, input_resolution=(16, 16), num_heads=8, window_size=4,
        ...
        self.WA1 = Window_Attention(dim=self.dim, input_resolution=input_resolution[0], num_heads=num_heads,
                                    window_size=window_size)
        self.WA2 = Window_Attention_shuffle(dim=self.dim, input_resolution=input_resolution[0], num_heads=num_heads,
                                         window_size=window_size)
        self.WA3 = Window_Attention(dim=self.dim, input_resolution=input_resolution[0], num_heads=num_heads,
                                             window_size=window_size)
...
if __name__ == '__main__':
    ...
    sz = 256
    input1 = torch.randn(1, 34, sz, sz).cuda()
    input2 = torch.randn(1, 31, sz // 4, sz // 4).cuda()
    ...
    t = Merge(img_size=sz, patch_size=1, in_chans1=34, in_chans2=31, embed_dim=48, num_heads1=[8, 8, 8],
              window_size=8, group=8, mlp_ratio=4., dim=48, num_heads2=[8, 8, 8], ffn_expansion_factor=2.66,
