...
class XCAttention(nn.Module):
    def __init__(self, dim, img_size, num_heads, win, bias):
        super(XCAttention, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
        self.win = win
        self.pooling = nn.AdaptiveAvgPool2d((img_size//2, img_size//2))
        self.up = Upsample_unc(dim)
        self.qkv = nn.Conv2d(dim, dim * 3, kernel_size=1, bias=bias)
        self.qkv_dwconv = nn.Conv2d(dim * 3, dim * 3, kernel_size=3, stride=1, padding=1, groups=dim * 3,
                                    bias=bias)

        self.qkv2 = nn.Conv2d(dim, dim * 3, kernel_size=1, bias=bias)
        self.qkv_dwconv2 = nn.Conv2d(dim * 3, dim * 3, kernel_size=3, stride=1, padding=1, groups=dim // 2 * 3,
                                    bias=bias)
    ...
    def forward(self, x):
        ...
        q_win = q.permute(0, 2, 4, 1, 3, 5).contiguous().view(-1, C // 2, self.win * self.win)

        k = k.view(B, C, H // self.win, self.win, W // self.win, self.win)
        k_win = k.permute(0, 2, 4, 1, 3, 5).contiguous().view(-1, C // 2, self.win * self.win)

        v = v.view(B, C, H // self.win, self.win, W // self.win, self.win)
        v_win = v.permute(0, 2, 4, 1, 3, 5).contiguous().view(-1, C // 2, self.win * self.win)
        ...
        q2 = q2.view(B, C, H // self.win, self.win, W // self.win, self.win)
        q_win2 = q2.permute(0, 2, 4, 1, 3, 5).contiguous().view(-1, C, self.win * self.win)

        k2 = k2.view(B, C, H // self.win, self.win, W // self.win, self.win)
        k_win2 = k2.permute(0, 2, 4, 1, 3, 5).contiguous().view(-1, C, self.win * self.win)

        v2 = v2.view(B, C, H // self.win, self.win, W // self.win, self.win)
        v_win2 = v2.permute(0, 2, 4, 1, 3, 5).contiguous().view(-1, C, self.win * self.win)

        q2 = rearrange(q_win2, 'n (head c) win -> n head c win', head=self.num_heads)
        k2 = rearrange(k_win2, 'n (head c) win -> n head c win', head=self.num_heads)
        v2 = rearrange(v_win2, 'n (head c) win -> n head c win', head=self.num_heads)
        ...
        out2 = rearrange(out2, 'n head c (h w) -> n (head c) h w', w=self.win * 2)
        out2 = out2.view(B, H // self.win, W // self.win, C, self.win, self.win)
        out2 = out2.permute(0, 3, 1, 4, 2, 5).contiguous().view(B, C, H, W)
        out2_up = self.up(out2)
        output = torch.cat((out, self.sigmoid(out2_up + short_cut)), 1)
...
class XCABlock(nn.Module):
    def __init__(self, dim, img_size, num_heads, win, ffn_expansion_factor, bias, LayerNorm_type):
        super(XCABlock, self).__init__()

        self.num_heads = num_heads

        self.norm1 = LayerNorm(dim, LayerNorm_type)
        self.attn = XCAttention(dim, img_size, num_heads, win, bias)
...
class Direction2(nn.Module):
    def __init__(self, img_size=64, in_channels=31, dim=32, num_heads=[8, 8, 8], group=8, ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False):
        super(Direction2, self).__init__()

        self.patch_embed = OverlapPatchEmbed(in_channels, dim)

        self.stage1 = XCABlock(dim=dim, img_size=img_size//4, num_heads=num_heads[0], win=group, ffn_expansion_factor=ffn_expansion_factor,
                                   bias=bias, LayerNorm_type=LayerNorm_type)
        self.up1_2 = Upsample_unc(int(dim))

        self.stage2 = XCABlock(dim=dim, img_size=img_size//2, num_heads=num_heads[1], win=group, ffn_expansion_factor=ffn_expansion_factor,
                          bias=bias, LayerNorm_type=LayerNorm_type)
        self.up2_3 = Upsample_unc(int(dim))

        self.stage3 = XCABlock(dim=dim, img_size=img_size, num_heads=num_heads[2], win=group, ffn_expansion_factor=ffn_expansion_factor,
                          bias=bias, LayerNorm_type=LayerNorm_type)
...
class Merge(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans1=34, in_chans2=31, embed_dim=32, num_heads1=[8, 8, 8],window_size=8, group=8, mlp_ratio=4.,dim=32,
        ...
        self.direction2 = Direction2(img_size=img_size, in_channels=in_chans2, dim=dim, num_heads=num_heads2, group=group, ffn_expansion_factor=ffn_expansion_factor,  LayerNorm_type=LayerNorm_type, bias=bias)
