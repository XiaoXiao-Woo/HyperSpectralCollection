...
#     def __init__(self, img_size=64, patch_size=1, in_chans1=34, in_chans2=31, embed_dim=48, num_heads1=[8, 8, 8],
#                        window_size=8, group=8,  mlp_ratio=4.,dim=32, num_heads2=[8, 8, 8],  ffn_expansion_factor=2.66,
...
class Merge(nn.Module):
    def __init__(self, img_size=5, patch_size=1, in_chans1=34, in_chans2=31, embed_dim=48, num_heads1=[8, 8, 8],window_size=8, group=8, mlp_ratio=4.,dim=32,
                 num_heads2=[8, 8, 8], ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False
                 ):

        super(Merge, self).__init__()
        self.direction1 = Direction1(img_size=img_size, patch_size=patch_size, in_chans=in_chans1, embed_dim=embed_dim,
                                     num_heads=num_heads1, window_size=window_size, mlp_ratio=mlp_ratio)
        self.direction2 = Direction2(inp_channels=in_chans2, dim=dim, num_heads=num_heads2, group=group, ffn_expansion_factor=ffn_expansion_factor,  LayerNorm_type=LayerNorm_type, bias=bias)
        self.merge1 = nn.Sequential(nn.Conv2d(embed_dim * 4, dim, kernel_size=3, padding=1, bias=bias),
                                    nn.Conv2d(dim, dim, kernel_size=5, padding=2, bias=bias),
                                    nn.LeakyReLU(0.2, bias))

        self.up1 = Upsample_unc(int(dim))

        self.merge2 = nn.Sequential(nn.Conv2d(embed_dim * 2 + dim, dim, kernel_size=3, padding=1, bias=bias),
                                    nn.Conv2d(dim, dim, kernel_size=5, padding=2, bias=bias),
                                    nn.LeakyReLU(0.2, bias))
    ...
    def forward(self, x, y):
        ...
        merge1 = self.merge1(D1_stage3)
        merge2 = self.merge2(torch.cat((D1_stage2, self.up1(merge1)), 1))
        output = self.merge3(torch.cat((D1_stage1, D2_stage3, self.up2(merge2)), 1))
