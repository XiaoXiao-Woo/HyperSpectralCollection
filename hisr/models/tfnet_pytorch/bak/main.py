...
import os
import time
from implict_representation import LIIF
...
parser.add_argument('--dataname', type=str, default='cave_x4')
parser.add_argument("--resume", default='model/restfnet_harvard_x4_202212071446/s.pth', type=str ,help="Path to checkpoint (default: none)")
...
parser.add_argument("--net", type=str, default='tfsnet', choices={'resnet','tfnet'})
...
def main():
    ...
    print("===> Setting GPU")
    if cuda:
        model = LIIF(encoder_spec=None, imnet_spec=None,
...
def train(training_data_loader, optimizer, model, criterion, epoch, train_log):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        # else:
        #     output = model(input_pan, input_lr_u)
        b, c, h, w = input_lr_u.shape
        coord = make_coord((h, w), flatten=False).expand(b, h, w, 2).cuda()  # BxHxWx2
        cell = torch.ones_like(coord).cuda()  # BxHxWx2
        cell[:, 0] *= 2 / h
        cell[:, 1] *= 2 / w
        output = model(input_pan, input_lr, coord, cell)
