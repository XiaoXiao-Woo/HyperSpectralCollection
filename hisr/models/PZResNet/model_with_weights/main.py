...
if __name__ == '__main__':
    ...
    parse.add_argument('--lr',type = float, default = 2e-3, help='learing rate of network')
    ...
    if conti == True:
        ...
        # renet = load_model(renet,state_dict['renet'])
        del state_dict
    for epoch in range(opt.n_epochs):
        ...
        for hsi,hsi_g,hsi_resize,msi in Hyper_train:
            ...
            # white = torch.zeros_like(msi[-1]).cuda().float()
            ...
            # if conti == True:
            #     loss =( MaeLoss(fout.to(device),hsi_g.to(device))+  MaeLoss(hsi_1.to(device),hsi_g.to(device)) + MaeLoss(hsi_2.to(device),hsi_g.to(device)))
