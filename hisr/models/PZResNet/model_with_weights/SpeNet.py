...
class BilinearConvTranspose2d(nn.ConvTranspose2d):
    ...
    def __init__(self, channels, stride, groups=31):
        """Set up the layer.
        Parameters
        ----------
        channels: int
        ...
        kernel_size = (2 * stride[0] , 2 * stride[1])
    ...
    def reset_parameters(self):
        ...
        bilinear_kernel = self.bilinear_kernel(self.stride).cuda().float()
        for i in range(self.in_channels):
            if self.groups == 1:
                j = i
            else:
                j = 0
            self.weight.data[i, j][:-1,:-1] = bilinear_kernel
    ...
    def bilinear_kernel(stride):
        ...
        shape = (1,) * num_dims
        bilinear_kernel = torch.ones(*shape).cuda().float()
        ...
        for channel in range(num_dims):
            ...
            channel_filter = channel_filter.cuda().float()
...
class denselayer(nn.Module):
    def __init__(self,cin,cout=31,RELU=True,BN=True):
        super(denselayer, self).__init__()
        self.compressLayer = BCR(kernel=1,cin=cin,cout=cout,RELU=True,BN=BN)
        self.actlayer = BCR(kernel=3,cin=cout,cout=cout,group=1,RELU=RELU,padding=1,BN=BN)
...
class stage(nn.Module):
    def __init__(self,cin,cout,final=False,extra=0):
        ...
        self.Upconv = BCR(kernel = 3,cin = cin, cout = cout,stride= 1,padding=1,BN=False)
        if final == True:
            f_cout = cout +1
        else:
            f_cout = cout
        ...
        self.denselayers = nn.ModuleList([
            denselayer(cin=2*cout+extra,cout=cout*2,BN=True),
            denselayer(cin=4*cout+extra,cout=cout*2,BN=True),
            denselayer(cin=6*cout+extra,cout=cout*2,BN=True),
            denselayer(cin=8*cout+extra,cout=cout*2,BN=True),
            denselayer(cin=10*cout+extra,cout=cout*2,BN=True),
            denselayer(cin=12*cout+extra,cout=cout*2,BN=True),
            denselayer(cin=14*cout+extra,cout=cout*2,BN=True),
            # denselayer(cin=16*cout+extra,cout=cout*2,BN=True),
            # denselayer(cin=18*cout+extra,cout=cout*2,BN=True),
            denselayer(cin=16*cout+extra,cout=f_cout,RELU=False,BN=True)])
...
class SpeNet(nn.Module):
    def __init__(self,extra=[0,0,0]):
        ...
        denselayer(cin = 31, cout= 31,BN=False),
        denselayer(cin = 31, cout= 31,BN=False),
        denselayer(cin = 31, cout= 31,BN=False,RELU=False))
        # self.transpose = BilinearConvTranspose2d(channels = 31,stride=8)
    def forward(self,HSI,MSI,extra_data=None):
        MSI = MSI - self.pool(MSI)
        # HSI = self.transpose(HSI)
        MSI = [MSI]
