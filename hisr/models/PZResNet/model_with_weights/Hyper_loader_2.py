...
class Hyper_dataset(Dataset):
    ...
    def __init__(self, output_shape=512,ratio = 1,Training_mode='Train',data_name = 'CAVE',use_generated_data = False, use_all_data = True):
        ...
        if data_name == 'CAVE':
            self.path = '/home/zzy/data/CAVEdata_1931/'
            ...
            name = scio.loadmat('/home/zzy/ablation/kernel_shape/3/final_name4.mat')
            ...
            self.train_path = '/public/nas/Hyperspectal_image_dataset/train/'
            file_name = os.walk(self.train_path)
            file_name = [i for i in file_name]
            self.train_name = file_name[0][2]
            self.test_path = '/public/nas/Hyperspectal_image_dataset/test/'
            ...
            # self.
            max_v = 0
            file_=h5py.File('/home/zhu_19/data/hyperspectral_image/data.h5','r')
            for name in self.train_name:
                temp_max = np.max( file_[name][:])
                max_v = max(max_v,temp_max)
            self.max_v = max_v
        self.reps = scio.loadmat('/home/zzy/ablation/kernel_shape/1/resp.mat')['resp']
    ...
    def __getitem__(self, index):
        if self.data_name == 'CAVE':
            ...
            if self.TM=='Train':
                ...
                # temp_a = cv2.GaussianBlur(temp,(31,31),8)[15::32,15::32,:]
                # hsi = temp_a[index_row*4:(index_row+1)*4,index_col*4:(index_col+1)*4,:]
                # temp_a = cv2.GaussianBlur(temp,(4,4),2)[0::4,0::4,:]
                temp_a = cv2.GaussianBlur(temp,(9,9),2)[3::8,3::8,:]
                ...
                hsi_g = hsi_g['msi']
                # hsi = cv2.GaussianBlur(hsi_g,(31,31),8)[15::32,15::32,:]
                # hsi = cv2.GaussianBlur(hsi_g,(5,5),2)[1::4,1::4,:]
                hsi = cv2.GaussianBlur(hsi_g,(9,9),2)[3::8,3::8,:]
                # hsi = temp_a[index_row*16:(index_row+1)*16,index_col*16:(index_col+1)*16,:]
                msi = np.tensordot(hsi_g,self.reps,(-1,0))
            ...
            file=h5py.File('/home/zhu_19/data/hyperspectral_image/data.h5','r')
            # file2=h5py.File('/home/zhu_19/data/hyperspectral_image/Lr_data.h5','r')
            if self.TM=='Train':
                ...
                hsi_g = hsi_g[index_row*128:(index_row+1)*128,index_col*128:(index_col+1)*128,:]/self.max_v
                hsi = cv2.GaussianBlur(hsi_g,(33,33),8)[15::32,15::32,:]
                # hsi_g = 
                ...
                hsi_g = hsi_g[0:512,0:512,:]/self.max_v
                hsi = cv2.GaussianBlur(hsi_g,(33,33),8)[15::32,15::32,:]
                # temp_a = cv2.GaussianBlur(hsi_g,(31,31),2)[15::32,15::32,:]
                # hsi_g = hsi_g[0:1024,0:1024,:]
                # hsi = temp_a[0:1024//32,0:1024//32,:]
                msi = np.tensordot(hsi_g,self.reps,(-1,0))
        ...
        return ((hsi,hsi_resize,hsi_resize), hsi_g, hsi_resize, (msi,msi,msi))
