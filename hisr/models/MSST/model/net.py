...
from HSI.SpeT import SpeTL
from MSI.SpaT import SpaTL
from timm.models.layers import trunc_normal_
from HSI.band_embed import BandEmbed
from MSI.patch_embed import PatchEmbed
...
class Net(nn.Module):
    ...
    def forward(self, y, z):
        ...
        fea_hsi_1 = self.norm_1(fea_hsi_1.permute(0, 2, 3, 1)).permute(0, 3, 1, 2) #[B, C, h, w]
        x_hsi_1 = self.band_unembed_1(fea_hsi_1) #[B, C, h, w]
        x_hsi_1 = self.conv_1(x_hsi_1)  # [B, C, h, w]
        ...
        fea_hsi_2 = self.norm_2(fea_hsi_2.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)  # [B, C/2, h, w]
        x_hsi_2 = self.band_unembed_2(fea_hsi_2)  # [B, C/2, h, w]
        x_hsi_2 = self.conv_2(x_hsi_2)  # [B, C, h, w]
        ...
        fea_hsi_3 = self.norm_3(fea_hsi_3.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)  # [B, C*2, h, w]
        x_hsi_3 = self.band_unembed_3(fea_hsi_3)  # [B, C*2, h, w]
        x_hsi_3 = self.conv_3(x_hsi_3)  # [B, C, h, w]
        ...
        embed_patch_16 = self.patch_embedding_16(x_1)   # [B, d, H, W]
        ...
        x_msi_16 = self.patch_unembed_16(fea_msi_16) #[B, C, H, W]
        x_msi_16 = self.conv_2(x_msi_16) #[B, C, H, W]
        ...
        fea_msi_8 = self.norm_8(fea_msi_8.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)  # [B, d, H/patch, W/patch]
        x_msi_8 = self.patch_unembed_8(fea_msi_8)  # [B, C, H, W]
        x_msi_8 = self.conv_8(x_msi_8)  # [B, C, H, W]
        ...
        fea_msi_32 = self.norm_32(fea_msi_32.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)  # [B, d, H/2, W/2]
        x_msi_32 = self.patch_unembed_32(fea_msi_32)  # [B, C, H, W]
        x_msi_32 = self.conv_32(x_msi_32)  # [B, C, H, W]
...
if __name__ == '__main__':
    ...
    args.ratio = 4
    _model = Net(args).cuda()
    torchstat.stat(_model, [[1, 31, 16, 16], [1, 3, 64, 64]])

    # args.ratio = 8
    # torchstat.stat(_model, [[1, 31, 16, 16], [1, 3, 128, 128]])


    HSI = torch.randn([1, 31, 16, 16]).cuda()
    MSI = torch.randn([1, 3, 16*args.ratio, 16*args.ratio]).cuda()
    GT = torch.randn([1, 31, 16*args.ratio, 16*args.ratio]).cuda()
    sr = _model(HSI, MSI)
    loss = F.mse_loss(sr, GT)
    loss.backward()
    sum_p = 0
    for m,p in _model.named_parameters():
