...
class LAGNET(nn.Module):
    def __init__(self, criterion, spectral_num, rgb_channel=3):
        super(LAGNET, self).__init__()

        self.criterion = criterion

        self.head_conv = nn.Sequential(
            LAGConv2D(spectral_num+rgb_channel, 32, 3, 1, 1, use_bias=True),
            nn.ReLU(inplace=True)
        )
...
class build_LAGNet(HISRModel, name='LAGNet'):
    def __call__(self, cfg):
        ...
        model = LAGNET(criterion, spectral_num, 3).cuda()
