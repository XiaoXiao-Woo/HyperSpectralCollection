...
def Win_Dila(x, win_size):
    ...
    xt[:, :, :, 0:n_win//3, 0:n_win//3, :] = x1  # B n/2 n/2 d2c
    xt[:, :, :, 0:n_win//3, n_win//3:n_win*2//3, :] = x2  # B n/2 n/2 d2c
    xt[:, :, :, 0:n_win//3, n_win*2//3:n_win, :] = x3  # B n/2 n/2 d2c
    xt[:, :, :, n_win//3:n_win*2//3, 0:n_win // 3, :] = x4  # B n/2 n/2 d2c
    xt[:, :, :, n_win//3:n_win*2//3, n_win // 3:n_win * 2 // 3, :] = x5  # B n/2 n/2 d2c
    xt[:, :, :, n_win//3:n_win*2//3, n_win * 2 // 3:n_win, :] = x6  # B n/2 n/2 d2c
    xt[:, :, :, n_win * 2 // 3:n_win, 0:n_win // 3, :] = x4  # B n/2 n/2 d2c
    xt[:, :, :, n_win * 2 // 3:n_win, n_win // 3:n_win * 2 // 3, :] = x5  # B n/2 n/2 d2c
    xt[:, :, :, n_win * 2 // 3:n_win, n_win * 2 // 3:n_win, :] = x6  # B n/2 n/2 d2c
...
def Win_ReDila(x, win_size):
    ...
    xt = torch.zeros_like(x)
    xt[:, :, :, 0::3, 0::3, :] = x[:, :, :, 0:n_win // 3, 0: n_win // 3, :]
    xt[:, :, :, 0::3, 1::3, :] = x[:, :, :, 0:n_win // 3, n_win // 3: n_win*2 // 3, :]
    xt[:, :, :, 0::3, 2::3, :] = x[:, :, :, 0:n_win // 3, n_win * 2 // 3: n_win, :]
    xt[:, :, :, 1::3, 0::3, :] = x[:, :, :, n_win // 3: n_win * 2 // 3, 0: n_win // 3, :]
    xt[:, :, :, 1::3, 1::3, :] = x[:, :, :, n_win // 3: n_win * 2 // 3, n_win // 3: n_win * 2 // 3, :]
    xt[:, :, :, 1::3, 2::3, :] = x[:, :, :, n_win // 3: n_win * 2 // 3, n_win * 2 // 3: n_win, :]
    xt[:, :, :, 2::3, 0::3, :] = x[:, :, :, n_win * 2 // 3: n_win, 0: n_win // 3, :]
    xt[:, :, :, 2::3, 1::3, :] = x[:, :, :, n_win * 2 // 3: n_win, n_win // 3: n_win * 2 // 3, :]
    xt[:, :, :, 2::3, 2::3, :] = x[:, :, :, n_win * 2 // 3: n_win, n_win * 2 // 3: n_win, :]
...
class Merge(nn.Module):
    ...
    def forward(self, x, y):
        ...
        y = F.pad(y, (pad//4, pad//4, pad//4, pad//4))
        ...
        output = output[:, :, pad:64+pad, pad:64+pad]
...
if __name__ == '__main__':
    ...
    # t = Direction2().cuda()
    t = Merge(img_size=96, patch_size=1, in_chans1=34, in_chans2=31, embed_dim=48, num_heads1=[8, 8, 8],
    ...
    num_params = 0
    for param in Merge(img_size=96, patch_size=1, in_chans1=34, in_chans2=31, embed_dim=48, num_heads1=[8, 8, 8],
                       window_size=8, group=8,  mlp_ratio=4.,dim=48, num_heads2=[8, 8, 8],  ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias',
