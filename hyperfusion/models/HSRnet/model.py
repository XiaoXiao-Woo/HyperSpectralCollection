...
class HSRNet(nn.Module):
    def __init__(self, spectral_num, num_res=6, num_feature=64):
        ...
        # (64 - k + 2*p)/4 + 1 = 16
        # 58/4 + p/2 = 15
        self.rgb_conv = nn.Conv2d(3, 3, kernel_size=6, stride=4, padding=1)  # kernel_size=6, stride=4

        self.rs_conv1 = nn.Conv2d(spectral_num + 3, spectral_num * 4 * 4, kernel_size=3, stride=1, padding=1)
        self.rs_conv2 = nn.Conv2d(spectral_num + 3, num_feature, kernel_size=3, stride=1, padding=1)
        ...
        self.rs_conv3 = nn.Conv2d(num_feature, spectral_num, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, ms, RGB):
        ...
        rs = torch.cat([_phase_shift(x, 4) for x in Xc], 1)  # each of x in Xc is r * r channel 分别每一个通道变为r*r
        Rslice, Gslice, Bslice = torch.split(RGB, [1, 1, 1], dim=1)
...
if __name__ == '__main__':
    ...
    # print(model(ms, RGB)[0].shape)
    torchstat.stat(model, [[1, 31, 16, 16], [1, 3, 64, 64]])
    # torchstat.stat(model, [[1, 31, 16, 16], [1, 3, 128, 128]])
    '''
    (first four is divided by 1000)
    Total params: 633.39K 0.6334M (633,390)
    Total memory: 223.80MB
    Total MAdd: 64.14GMAdd 0.0641TMAdd
    Total Flops: 2.01GFlops 0.002TFlops
    Total MemR+W: 444.24MB 0.4338GB
    '''