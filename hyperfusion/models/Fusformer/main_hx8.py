...
model_folder = "Trained_model/harvard_x8/"
...
def train(training_data_loader, validate_data_loader=0, start_epoch=0, RESUME=False):
...
def test():
    test_set = DatasetFromHdf5("F:/Data/HSI/harvard_x8/test_harvard(with_up)x8_rgb40.h5")
    print(torch.cuda.get_device_name(0))
    num_testing = 10*25*25
    ...
    print('[Network %s] Total number of parameters : %.3f M' % ('fusformer', num_params / 1e6))
    ...
    for iteration, batch in enumerate(testing_data_loader, 1):
        ...
        HRMSI = HRMSI.cuda()
        with torch.no_grad():
            output_HRHSIone,_,_ = model(LRHSI, HRMSI)
    ...
    sio.savemat('harvard10_x8-fusformer25.mat',{'output': output_HRHSI})
###################################################################
# ------------------- Main Function  -------------------
###################################################################
if __name__ == '__main__':
    ...
    if train_or_not:
        ...
        train_set = DatasetFromHdf5('F:/Data/HSI/harvard_x8/train_harvard(with_up)x8_rgb.h5')  # creat data for training
        training_data_loader = DataLoader(dataset=train_set, num_workers=0, batch_size=batch_size, shuffle=True,
                                          pin_memory=True, drop_last=True)  # put training data to DataLoader for batches
        # validate_set = DatasetFromHdf5('/home/ShangqiDeng/data/HSI/cave_x8/validation_cave(with_up)x8_rgb.h5')  # creat data for validation
        # validate_data_loader = DataLoader(dataset=validate_set, num_workers=0, batch_size=batch_size, shuffle=True,
        #                                   pin_memory=True, drop_last=True)  # put training data to DataLoader for batches

        # train(training_data_loader=training_data_loader, validate_data_loader=validate_data_loader) #, start_epoch=200)  # call train function (call: Line 53)
        train(training_data_loader=training_data_loader)  # , start_epoch=200)  # call train function (call: Line 53)
