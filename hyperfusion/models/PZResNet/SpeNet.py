...
class SpeNet(nn.Module):
    def __init__(self, spectral_num, rgb_channel, extra=[0, 0, 0]):
        super(SpeNet, self).__init__()
        self.stages = nn.ModuleList([
            stage(cin=rgb_channel, cout=8, extra=extra[0]),
            stage(cin=8 + rgb_channel, cout=16, extra=extra[1]),
            stage(cin=16 + rgb_channel, cout=spectral_num, extra=extra[2], final=True)])
        self.scale_learning = scale_learning()
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.refine = nn.Sequential(
            denselayer(cin=spectral_num, cout=spectral_num))
    ...
    def forward(self, HSI, MSI, extra_data=None):
        ...
        for index, stage in enumerate(self.stages):
            ...
            MSI.append(msi_)
        out = MSI[-1][:, :31, :, :] + HSI
        refined_hsi = self.refine(out) + out
        return out, refined_hsi


    def train_step(self, data):
        log_vars = {}
        gt, up_hp, hsi, msi = data['gt'].cuda(), \
                           data['up'].cuda(), \
                           data['lrhsi'].cuda(), \
                           data['rgb'].cuda()
        # ratio = gt.shape[1] // msi.shape[1]

        hsi_spe, refined = self(up_hp, msi)
        loss = self.criterion(hsi_spe, gt)['loss'] + self.criterion(refined, gt)['loss']

        # with torch.no_grad():
        #     log_vars.update(analysis_accu(gt.cuda(), sr.cuda(), ratio, choices=4))
        #     log_vars.update(loss=loss.item())

        return refined, loss#{'loss': loss, 'log_vars': log_vars}


    def val_step(self, x, gt):
...
class build_SpeNet(HISRModel, name='PZResNet'):
...
if __name__ == '__main__':
    ...
    HSI = torch.randn([1, 31, 64, 64]).cuda()
    MSI = torch.randn([1, 3, 64, 64]).cuda()
    mod = SpeNet(31, 3).cuda()

    print(mod(HSI, MSI).shape)
